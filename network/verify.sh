...
check_contains vps_init_tool.sh 'TOOL_VERSION="1\.0\.14"' 'vps_init_tool.sh version drifted'
check_not_contains vps_init_tool.sh '涓|锛|銆|鑺|绔|鏈|璇|妫|瀹|鍩|闃|鍚|鎵|榛|浣|寤' 'vps_init_tool.sh must not contain mojibake Chinese text'
check_not_contains vps_init_tool.sh '�' 'vps_init_tool.sh must not contain replacement characters from partial text recovery'
...
check_contains vps_init_tool.sh 'bash vps_init_tool.sh --baseline --yes' 'vps_init_tool.sh recommended workflow must include low-risk baseline'
...
check_contains freedom.sh 'SCRIPT_VERSION="v9\.17-xray"' 'freedom.sh version drifted'
...
check_contains freedom.sh 'STABLE keeps the XHTTP server on TCP H1/H2' 'freedom.sh help must explain the stable XHTTP ALPN default'
check_contains freedom.sh 'Direct H3 makes Xray listen on UDP/443' 'freedom.sh help/preflight must warn about direct Xray H3'
check_contains freedom.sh 'Prefer real Nginx/Caddy/CDN fronting for H3' 'freedom.sh H3 warning should recommend fronting direct H3'
...
check_contains freedom.sh 'check_tls_cert_files \|\| fatal "TLS certificate validation failed"' 'freedom.sh deploy-time TLS cert validator must fail hard through wrapper'
check_contains freedom.sh 'check_tls_cert_files \|\| failures=\$\(\(failures \+ 1\)\)' 'freedom.sh preflight must use non-fatal TLS certificate checks'
check_contains freedom.sh 'domain_matches_certificate_name\(\)' 'freedom.sh missing TLS certificate domain matcher'
