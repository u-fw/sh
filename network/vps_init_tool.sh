...
# VPS Init Tool v1.0.0
# Debian/Ubuntu VPS bootstrap, audit and maintenance helper.
# Scope: memory, SSH, UFW firewall, DNS, logs, basic network tuning.
# Principle: audit first, confirm before any risky change.
...
show_system_status() {
  ...
  free -h || true
  echo
  swapon --show --output=NAME,TYPE,SIZE,USED,PRIO 2>/dev/null || swapon --show || true
  echo
  ...
  blue "===== Listening ports ====="
  ss -tulpen 2>/dev/null | head -120 || true
  echo

  blue "===== BBR / qdisc ====="
  sysctl net.ipv4.tcp_congestion_control 2>/dev/null || true
  sysctl net.core.default_qdisc 2>/dev/null || true
  echo "available: $(cat /proc/sys/net/ipv4/tcp_available_congestion_control 2>/dev/null || true)"
}
...
memory_report() {
  ...
  blue "===== Memory audit ====="
  echo "RAM: ${mem_mb} MB"
  echo "Recommended swapfile: $(recommend_swap_size)"
  echo "Recommended ZRAM: $(recommend_zram_size)"
  echo "Recommended vm.swappiness: $(recommend_swappiness)"
  echo "Recommended vm.vfs_cache_pressure: 50"
  echo
  free -h || true
  echo
  swapon --show --output=NAME,TYPE,SIZE,USED,PRIO 2>/dev/null || swapon --show || true
  ...
  echo
  blue "Detected ZRAM configs/services"
  ls -lh /etc/systemd/zram-generator.conf /etc/default/zramswap /etc/systemd/system/zram-swap.service 2>/dev/null || true
  systemctl is-active systemd-zram-setup@zram0.service 2>/dev/null && echo "systemd-zram-generator: active" || true
  systemctl is-active zramswap.service 2>/dev/null && echo "zramswap: active" || true
  systemctl is-active zram-swap.service 2>/dev/null && echo "fallback zram-swap.service: active" || true
  echo
  if [ "$mem_mb" -le 2048 ]; then
    yellow "Recommendation: small VPS profile: ZRAM + swapfile, swappiness 10."
  elif [ "$mem_mb" -le 8192 ]; then
    yellow "Recommendation: medium VPS profile: optional ZRAM + swapfile, swappiness 10."
  else
    yellow "Recommendation: large VPS profile: usually swapfile only, ZRAM optional, swappiness 1."
  fi
}
...
setup_swapfile() {
  ...
  if [[ "$size" =~ ^0([MGmg])?$ ]]; then yellow "Swapfile skipped."; return 0; fi
...
setup_zram() {
  ...
  [ "$enable" = "yes" ] || { yellow "ZRAM skipped."; return 0; }
  ...
  while true; do
    ...
    case "$c" in
      ...
      4)
        ...
        green "VM memory sysctl applied."; pause ;;
      5)
        memory_report
...
ssh_effective() {
  sshd -T 2>/dev/null | grep -E '^(port|permitrootlogin|passwordauthentication|kbdinteractiveauthentication|pubkeyauthentication|maxauthtries|maxsessions|maxstartups|logingracetime|permitemptypasswords|clientaliveinterval|clientalivecountmax|x11forwarding|allowagentforwarding|allowtcpforwarding|permittunnel|gatewayports|allowusers) ' || true
}

ssh_audit() {
  blue "===== SSH audit ====="
  if ! has_cmd sshd; then red "sshd not found."; return 1; fi
  ssh_effective
  echo
  local port pass root pub maxauth
  ...
  [ "$port" = "22" ] && yellow "Notice: SSH is on port 22. This is acceptable with key-only + Fail2ban, but noisy."
  [ "$pass" = "yes" ] && yellow "Recommendation: add/verify public key, then disable password login."
  [ "$root" = "yes" ] && yellow "Recommendation: avoid root password login; use prohibit-password or a non-root sudo user."
  [ "$pub" != "yes" ] && red "Warning: PubkeyAuthentication is not enabled."
  if [[ "$maxauth" =~ ^[0-9]+$ ]] && [ "$maxauth" -gt 3 ]; then yellow "Recommendation: set MaxAuthTries 3."; fi
}
...
ufw_install() { apt_install ufw; green "UFW installed."; }
...
ufw_audit() {
  ...
  if has_cmd ufw && ufw status | grep -q inactive; then
    yellow "Recommendation: enable UFW after allowing SSH port $ssh_port/tcp."
  fi
  yellow "Recommendation: restrict admin panels such as 3x-ui to your management IP/CIDR."
}
...
fail2ban_audit() {
  blue "===== Fail2ban audit ====="
  if ! has_cmd fail2ban-client; then yellow "Fail2ban not installed."; return 0; fi
  fail2ban-client status || true
  fail2ban-client status sshd 2>/dev/null || yellow "sshd jail not active or not found."
}
...
dns_audit() {
  blue "===== DNS audit ====="
  echo "/etc/resolv.conf -> $(readlink -f /etc/resolv.conf 2>/dev/null || echo plain-file)"
  cat /etc/resolv.conf 2>/dev/null || true
  echo
  if has_cmd resolvectl; then resolvectl status 2>/dev/null || true; fi
  if has_cmd systemd-resolve; then systemd-resolve --status 2>/dev/null || true; fi
  echo
  yellow "Recommendation: test resolvers before applying. Do not change DNS blindly on production servers."
}
...
logs_audit() {
  ...
  if is_systemd; then
    ...
    echo
    systemctl cat systemd-journald 2>/dev/null | sed -n '1,120p' || true
  else
    yellow "systemd not detected; journald audit skipped."
  fi
  echo
  blue "/var/log size"
  du -sh /var/log 2>/dev/null || true
  echo
  blue "Largest /var/log entries"
  du -ah /var/log 2>/dev/null | sort -hr | head -20 || true
}
...
audit_all() {
  clear
  blue "===== Full environment audit ====="
  show_system_status
  echo; memory_report
  echo; ssh_audit || true
  echo; ufw_audit || true
  echo; fail2ban_audit || true
  echo; dns_audit || true
  echo; logs_audit || true
}
